...
remote_state {
  ...
  config = {
    bucket = local.deployment_commons.terraform_state_s3_bucket

    key = "${path_relative_to_include()}/terraform.tfstate"
    region         = local.deployment_commons.terraform_state_aws_region
    encrypt        = true
    dynamodb_table = local.deployment_commons.dynamodb_table
  }
}

inputs = { 
  allowed_account_ids = local.deployment_commons.local.aws_account_id
}
